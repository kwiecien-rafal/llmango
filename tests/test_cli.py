...
def test_large_run_is_refused_without_force() -> None:
    result = runner.invoke(app, ["run", "favorite_fruit", "--samples", "100"])
...
def test_dry_run_reports_the_plan_and_writes_nothing(data_dirs: Path) -> None:
    result = runner.invoke(
        app, ["run", "favorite_fruit", "--dry-run", "--samples", "3"]
    )

    assert result.exit_code == 0
    assert "Dry run for favorite_fruit" in result.output
    assert "requests:  6 total" in result.output
...
def test_report_normalize_shows_parquet_when_written(
    ...
    _report_normalize(
        NormalizeOutcome(
            parquet_path=tmp_path / "favorite_fruit.parquet",
            rows=8,
            distinct=7,
            llm_calls=0,
        )
